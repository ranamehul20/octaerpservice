  ...
  try {
    ...
    if (!decode) {
      ...
      try {
        ...
        const user = await User.findOne({ _id: Schema.Types.ObjectId.createFromHexString(decoded.id), isDeleted: false });
        if (!user) {
          return res.status(404).json(errors("User not found!", res.statusCode));
        } else {
          const userDetails = await UserDetails.findOne({
            userId: Schema.Types.ObjectId.createFromHexString(decoded.id),
          }).populate("userId");
          if (!userDetails)
            return res.status(404).json(errors("User not found!", res.statusCode));
    ...
    const user = await User.findOne({ _id: Schema.Types.ObjectId.createFromHexString(decode.id), isDeleted: false });
    if (!user) {
      return res.status(404).json(errors("User not found!", res.statusCode));
    } else {
      const userDetails = await UserDetails.findOne({
        userId: Schema.Types.ObjectId.createFromHexString(decode.id),
      }).populate("userId");
      if (!userDetails)
        return res.status(404).json(errors("User not found!", res.statusCode));
      req.userDetails = userDetails;
...
export const noticeValidator = async (notice) => {
  ...
  if (notice.blockId && Array.isArray(notice.blockId)) {
    const invalidBlocks = await Promise.all(
      notice.blockId.map(async (block) => {
        try {
          const blockExists = await BlockMst.findOne({ _id: block });
    ...
    const notFoundBlocks = invalidBlocks.filter((block) => block !== null);
    if (notFoundBlocks.length > 0) {
      error.blocks = `Blocks not found: ${notFoundBlocks.join(", ")}`;
    }
  }
  return {
    error,
